...
async def login_for_access_token(
    ...
    #  access token with user id as subject
...
async def get_user_posts(
    user_id: int,
    db: Annotated[AsyncSession, Depends(get_db)],
    skip: Annotated[int, Query(ge=0)] = 1,
    ...
    count_result = await db.execute(
        select(func.count()).select_from(models.Post).where(models.Post.id == user_id)
    )
...
async def upload_profile_picture(
    ...
    try:
        ...
        raise HTTPException(
            ...
            detail="Failed to upload imahe. Please try again",
        )
